...
public struct Preset {
    ...
    /// 完整 system prompt（含角色前缀），单 skill 模式使用
    public let systemPrompt: String
    /// 纯规则正文（不含角色前缀），多 skill 合并时使用
    public let ruleBlock: String
...
public struct PresetManager {
    ...
    /// 直接拼接各 skill 的 ruleBlock，不做行级解析，保证完整性
    public static func buildCombinedPreset(enabledSkillIds: [String]) -> Preset? {
        ...
        // 只有一个 skill 时直接返回（使用完整 systemPrompt）
        if enabledPresets.count == 1 { return enabledPresets[0] }

        // 多个 skill 时合并：统一前缀 + 各 skill 的 ruleBlock 直接拼接
        ...
        let taskBlocks = enabledPresets.enumerated().map { i, preset in
            "\n\n【任务\(i + 1): \(preset.name)】\n\(preset.ruleBlock.trimmingCharacters(in: .whitespacesAndNewlines))"
        }
